...
class MAB_sell(object): ## added by me
    """
    Multi-armed bandit test class.
    """
    ...
    def __init__(
        ...
        self.wins: np.ndarray = np.zeros(num_bandits)
        self.pulls: np.ndarray = np.zeros(num_bandits)
        self.sell_no: np.ndarray = np.zeros(num_bandits) ## added by me
        self.pull_no: np.ndarray = np.zeros(num_bandits) ## added by me
    ...
    def _run(self, strategy: str, parameters: Optional[Dict] = None) -> None:
        ...
        choice: int = self.run_strategy(strategy, parameters)
        self.choices.append(choice)
        payout: Optional[int] = self.bandits.sell(choice)
        if payout is None:
            print("Trials exhausted. No more values for bandit", choice)
            return None
        elif payout == 1:
            self.pull_no[choice] += 1
        elif payout < 1 and payout > 0:
            self.sell_no[choice] += 1
        else:
            self.wins[choice] += payout
        self.pulls[choice] += 1
...
class Bandits:
    ...
    def __init__(
        ...
        if not live:
            ...
            self.root = int = root ## added by me
        else:
            self.live = True
            self.probs = None
    ...
    def sell(self, i: int) -> Optional[int]: ## added by me
        rand_n = np.random.rand()
        util = np.sqrt(self.probs[i]) ## added by me
        if self.live:
            if len(self.payouts[i]) > 0:
                return self.payouts[i].pop()
            else:
            ...
            if self.probs is None:
                return None
            elif (rand_n)**(1/self.root) > self.probs[i]: ###added by me
                return self.probs[i]
            elif rand_n < self.probs[i]:
                return 1
            else:
                return 0
